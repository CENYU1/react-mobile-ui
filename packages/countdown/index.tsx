...
const Countdown: React.FC<CountdownProps> = React.memo((props) => {
  ...
  return (
    <div className="cenyu-countdown">
      {timeEnd && props.endText ? (
        <div className={props.endTextClassName}>{props.endText}</div>
      ) : (
        timeItems.map((item, index) => (
          <div className="cenyu-countdown-item" key={index}>
            <div className={cx('cenyu-countdown-item-num', props.numberClassName)}>{item.num}</div>
            <div className={cx('cenyu-countdown-item-symbol', props.symbolClassName)}>{item.symbol}</div>
          </div>
        ))
      )}
    </div>
