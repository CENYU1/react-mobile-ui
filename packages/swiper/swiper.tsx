...
const Swiper = React.forwardRef<SwiperRef, SwiperProps>((props, ref) => {
  ...
  const renderSwiperItem = React.useCallback(() => {
    return (
      <div className="cenyu-swiper-track-inner">
        {React.Children.map(validChildren, (child, index) => {
          const position = getFinalPosition(index);

          return (
            <div
              className="cenyu-swiper-slide"
              style={{
  ...
  return (
    <div className="cenyu-swiper" style={props.style}>
      <div className="cenyu-swiper-track" ref={trackRef} onTouchStart={onTouchStart} onMouseDown={onTouchStart}>
        {renderSwiperItem()}
      </div>
      {props.showIndicator && (
        <div className="cenyu-swiper-indicator">
          <SwiperPageIndicator
            total={count}
